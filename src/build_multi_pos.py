...
class BuildMultiPos(python_strategy):
    ...
    def __init__(self):
        ...
        self.desired_pos = pd.Series([]) # dataframe
    ...
    def init(self):
        ...
        # get symbols
        self.param.symbols = df_signal.keys()

        # contract summary 显示单个合约持仓信息
        kt_info('Contract Summary:')
        for symbol in self.param.symbols:
            pos = self.api.get_instrument_position_detail(symbol)
            ...
            kt_info('合约:{}, 多仓:{}, 昨多:{}, 空仓:{}, 昨空:{}, 净多仓:{}'.format(symbol, total_long, history_long, total_short, history_short, init_net_pos))

        if action_day in df_signal.index:
            self.desired_pos = df_signal.loc[action_day]
            kt_info("update target pos from signal file: {}".format(self.desired_pos))
        else:
            kt_info("No existing day {} in the signal table.".format(action_day))
            self.desired_pos = pd.Series({x:0 for x in df_signal.keys()}) # dataframe
    ...
    def on_tick(self, t):
        # 对每一个Tick事件，做出相应交易动作
        # 对每一个symbol建仓
        if not t.instrument_id in self.param.symbols:
            return 
        ...
        if t.timestamp > build_start and t.timestamp < build_end:
            self.target_open.instrument_id = t.instrument_id
            self.target_open.algorithm = target_position_algorithm.basic
            self.target_open.target_pos = int(self.desired_pos[t.instrument_id])
        ...
        pos_summary = self.api.get_instrument_summary(t.instrument_id)
