...
class HoldOvernight(python_strategy):
    # HoldOvernight 实现跨交易日持仓类型的策略。e.g. 14：30 建仓，第二天09：30减仓。
    def __init__(self):
    ...
    def on_tick(self, t):
        ...
        build_start = parse_time(cur_td, self.param.build_start)
        build_end = parse_time(cur_td, self.param.build_end)
        reduce_start = parse_time(next_td, self.param.reduce_start)
        reduce_end = parse_time(next_td, self.param.reduce_end)
